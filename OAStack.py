...
try:
	settings = np.loadtxt('OAStack_settings.txt', dtype='str', delimiter='\t')
except (IOError):
	print "Cannot find OAStack_settings.txt. Either setup.py has not been run, or OAStack.py has been moved from where setup.py was run"
	sys.exit()
...
# unoriented stacking
	...
	# tsZ maps
		...
		# COOP peak interface
			...
			if mask_name:
				fname = "{}_stack_{}_masked_{}_nu{}".format(map_type, map_name[:-5], peak_type, int(nu))
			else:
				fname = "{}_stack_{}_{}_nu{}".format(map_type, map_name[:-5], peak_type, int(nu))
			...
			print "\nStacking...\n"
			if mask_name:
				print subprocess.check_output("{}/mapio/Stack -map {} -out {} -peaks {} -field T -mask {} -colortable 'jet'".format(COOP_dir, map_dir+'/'+map_name, stack_dir+'/out/'+fname, stack_dir+'/out/'+fname+'.dat', map_dir+'/'+mask_name), stderr=subprocess.STDOUT, shell=True)
			else:
				print subprocess.check_output("{}/mapio/Stack -map {} -out {} -peaks {} -field T -colortable 'jet'".format(COOP_dir, map_dir+'/'+map_name, stack_dir+'/out/'+fname, stack_dir+'/out/'+fname+'.dat'), stderr=subprocess.STDOUT, shell=True)
			print "\nGenerating PDF...\n"
